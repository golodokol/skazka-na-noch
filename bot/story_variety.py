...
HERO_ROLES = [
    "Ребёнок — главный герой; любимый герой из профиля — его постоянный напарник.",
    "Ребёнок принимает решения; любимый герой рядом в ключевых сценах.",
...
def pick_variety(
    ...
    *,
    ...
    fresh_boost = len(memories) + (3 if force_fresh else 0) + max(0, fresh_boost_offset)
...
def variety_user_block(
    ...
    *,
    ...
    lines = [
        ...
        "",
        f"ЛЮБИМЫЙ ГЕРОЙ «{hero_label}» — напарник {child_name}:",
        f"— «{hero_label}» в большинстве сцен, особенно в начале и финале; по имени 3–5 раз.",
        f"— 2–4 короткие тёплые реплики — свой голос, не поучения.",
        f"— Не заменять «{hero_label}» гостем сюжета и не убирать из текста.",
        "",
        f"Сеттинг (обязательно): {plan.setting}",
        f"Гость сюжета (одна встреча, ДОПОЛНИТЕЛЬНО к «{hero_label}»): {plan.helper}",
        f"Событие (покажи в действии и диалогах): {plan.angle}",
        f"Образ (вплети в сцену — см. также system prompt): {plan.metaphor}",
        f"Роли: {plan.hero_role}",
        "Без служебных слов: «метафора», «сюжет был», «символ» — только живая проза.",
        plan.avoid_block,
    ]
    if force_fresh:
        lines.insert(
